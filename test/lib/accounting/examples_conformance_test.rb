...
class ExamplesConformanceTest < Minitest::Test
  ...
  def setup
    @engine = Accounting::Engine.new
    {
      ...
      "FX"      => "expense"
    }.each { |code, type| @engine.add_account(code, code.tr("_", " ").capitalize, type) }
  end
  ...
  test "every documented example proves its classification" do
    EXAMPLES.each do |number, (description, raw)|
      ...
      entries.each_with_index do |lines, index|
        result = @engine.check(classified_lines(lines))
        ...
        lines.zip(result.proof.lines).each do |(code, type, side, _amount, effect), proof_line|
          assert_equal code, proof_line.account_code, label
  ...
  test "every documented example preserves the accounting equation when posted" do
    EXAMPLES.each do |number, (description, raw)|
      entries = entries_for(raw)

      entries.each_with_index do |lines, index|
        @engine.post_lines(
          date: Date.new(2026, 8, 1) + number,
          description: entry_label(number, description, index, entries.size),
          lines: classified_lines(lines)
        )

        assert @engine.equation_holds?,
          "Equation broken after #{entry_label(number, description, index, entries.size)}"
      end
    end

    assert_equal BigDecimal("324000"), @engine.balance("income")
  end
  ...
  def classified_lines(lines)
    lines.map do |code, type, side, amount|
      { account_code: code, base_type: type, side: side.to_sym, amount: BigDecimal(amount) }
    end
  end
end
